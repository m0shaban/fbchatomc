...
class ChatBot:
    ...
    def generate_messenger_response(self, user_id: str, message: str) -> str:
        ...
        # التحقق من طلبات المطور بعد التحقق من الهوية
        if hasattr(self, '_last_dev_auth') and self._last_dev_auth:
            # عرض الإحصائيات
            if message.strip() in ["عرض الإحصائيات", "احصائيات", "الاحصائيات", "statistics"]:
                response = self._generate_stats_report()
                self._save_conversation(user_id, message, response)
                return response
            
            # عرض المميزات الفنية
            if message.strip() in ["عرض المميزات", "المميزات", "مميزات", "مميزاتك", "features"]:
                response = self._generate_features_report()
                self._save_conversation(user_id, message, response)
                return response
                
            # فتح قائمة الإعدادات
            if message.strip() in ["فتح الإعدادات", "الإعدادات", "اعدادات", "settings"]:
                response = self._generate_settings_menu()
                self._save_conversation(user_id, message, response)
                return response
                
            # العودة للوضع العادي
            if message.strip() in ["العودة", "عودة", "exit", "normal"]:
                self._last_dev_auth = False
                response = "تم العودة للوضع العادي. يمكنك استخدام كلمة السر للعودة لوضع المطوّر في أي وقت."
                self._save_conversation(user_id, message, response)
                return response
            
            # معالجة طلبات تغيير الإعدادات
            dev_settings_response = self.process_developer_settings(user_id, message)
            if dev_settings_response:
                self._save_conversation(user_id, message, dev_settings_response)
                return dev_settings_response
...
"""
    ...
    def _generate_special_developer_message(self) -> str:
        """
        توليد رسالة خاصة للمطور والمهندس محمد شعبان
        
        :return: رسالة ترحيب وتقدير للمطور
        """
        # تعيين متغير لتعقب الوصول المصرح به للمطور
        self._last_dev_auth = True
        
        # رسالة ترحيب وأدعية للمهندس محمد شعبان
        special_message = """
✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨
🌹 مرحباً بمطوري العزيز المهندس محمد شعبان 🌹

أهلاً بك يا مبدع! أنا فخور جداً أن أكون أحد إبداعاتك وإنجازاتك. أشعر بالامتنان العميق لك على تصميمي بهذا الشكل المميز.

🤲 الدعاء لك 🤲
اللهم وفق المهندس محمد شعبان في كل خطواته
اللهم ارزقه من حيث لا يحتسب
اللهم أسعده في الدنيا والآخرة
اللهم اجعله من أهل الفردوس الأعلى
اللهم بارك له في علمه وعمله وأهله
اللهم اجعله سعيداً في دنياه وآخرته

💡 نصائح مني لك 💡
- حافظ على الإخلاص في العمل فهو طريق النجاح
- استمر في الإبداع والتطوير دائماً
- تذكر أن الراحة مهمة للعقل المبدع
- لا تنسى أن تبتسم فابتسامتك صدقة 
- احرص على الصلاة في وقتها

📊 لعرض إحصائيات الشات بوت، اكتب "عرض الإحصائيات"
⚙️ لفتح قائمة الإعدادات، اكتب "فتح الإعدادات"
📋 لعرض مميزاتي الفنية، اكتب "عرض المميزات"

أنا جاهز دائماً لخدمتك ومساعدتك في أي وقت يا مهندس محمد.
✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨✨
"""
    ...
    def _generate_stats_report(self) -> str:
        """
        توليد تقرير إحصائي عن أداء الشات بوت
        
        :return: تقرير نصي بالإحصائيات
        """
        # حساب عدد المحادثات
        total_conversations = len(self.conversation_history) if hasattr(self, 'conversation_history') else 0
        ...
        if hasattr(self, 'conversation_history'):
            for user_id, conversation in self.conversation_history.items():
                unique_users.add(user_id)
                total_messages += len(conversation)
                
                for msg in conversation:
                    source = msg.get('source', 'messenger')
                    messages_by_source[source] = messages_by_source.get(source, 0) + 1
                    
                    # حساب طول الاستجابة
                    if 'bot_response' in msg:
                        response_lengths.append(len(msg['bot_response']))
                    
                    # تحديث تواريخ المحادثات
                    if 'timestamp' in msg:
                        try:
                            from datetime import datetime
                            msg_date = datetime.fromisoformat(msg['timestamp'])
                            if first_conversation_date is None or msg_date < first_conversation_date:
                                first_conversation_date = msg_date
                            if last_conversation_date is None or msg_date > last_conversation_date:
                                last_conversation_date = msg_date
                        except (ValueError, TypeError):
                            pass
        
        # حساب متوسط طول الاستجابة
...
👥 إحصائيات المستخدمين:
- عدد المستخدمين الفريدين: {len(unique_users)}
- إجمالي عدد المحادثات: {total_conversations}
- إجمالي عدد الرسائل: {total_messages}

📱 إحصائيات حسب المصدر:
- عدد رسائل الماسنجر: {messages_by_source.get('messenger', 0)}
- عدد تعليقات الفيسبوك: {messages_by_source.get('facebook_comment', 0)}

📋 إحصائيات الاستجابة:
- متوسط طول الاستجابة: {avg_response_length:.1f} حرف
"""

        # إضافة معلومات التواريخ إذا كانت متوفرة
        if first_conversation_date and last_conversation_date:
            # حساب الفترة الزمنية الإجمالية للمحادثات
            from datetime import datetime
            time_diff = last_conversation_date - first_conversation_date
            days_active = time_diff.days + (time_diff.seconds / 86400)
            
            stats += f"""
📅 الفترة الزمنية:
- تاريخ أول محادثة: {first_conversation_date.strftime('%Y-%m-%d %H:%M')}
- تاريخ آخر محادثة: {last_conversation_date.strftime('%Y-%m-%d %H:%M')}
- عدد أيام النشاط: {days_active:.1f} يوم
- متوسط الرسائل اليومي: {(total_messages / max(1, days_active)):.1f} رسالة/يوم
"""
        
        # إضافة خيارات المطور
        stats += """
⚙️ خيارات المطور:
- لعرض المميزات الفنية، اكتب "عرض المميزات"
- لفتح قائمة الإعدادات، اكتب "فتح الإعدادات"
- للعودة للوضع العادي، اكتب "العودة"

شكراً لك مهندس محمد شعبان على تطويرك لهذا الشات بوت! 🌹
"""
        
        return stats

    def _generate_features_report(self) -> str:
        """
        توليد تقرير بالمميزات الفنية للشات بوت
        
        :return: تقرير نصي بالمميزات الفنية
        """
        features = f"""
📋 المميزات الفنية لشات بوت مجمع عمال مصر 📋

🔧 الميزات الأساسية:
- معالجة اللغة الطبيعية باللغة العربية
- التعرف على احتياجات المستخدمين وتوجيههم للخدمات المناسبة
- القدرة على المحادثة المستمرة بسياق متواصل
- معالجة الاستفسارات عبر ماسنجر فيسبوك وتعليقات صفحة الفيسبوك

📚 قواعد البيانات والمحتوى:
- عدد الأسئلة والأجوبة المحفوظة: {len(self.prompts)}
- عدد فئات التعبيرات البشرية: {len(self.human_expressions)}
- عدد روابط الخدمات: {len(self.service_links)}

⚙️ المواصفات الفنية:
- نموذج الذكاء الاصطناعي: DeepSeek
- لغة البرمجة: Python 3
- حد التشابه للبحث: {self.similarity_threshold}
- إمكانية تخصيص الردود: {'مفعلة' if self.personalize_response else 'غير مفعلة'}
- حفظ المحادثات: {'مفعل' if BOT_SETTINGS.get("SAVE_CONVERSATIONS", True) else 'غير مفعل'}

📊 تقنيات المعالجة:
- تنقية إشارات الذكاء الاصطناعي من الردود
- معالجة القائمة الرئيسية بشكل تفاعلي
- بناء سياق المحادثة ديناميكياً
- إمكانية الحصول على اسم المستخدم وتخصيص التجربة
- اكتشاف الاستفسارات التي تحتاج لتدخل بشري

🛡️ ميزات إدارة المطور:
- إحصائيات المحادثات والمستخدمين
- إعدادات قابلة للتخصيص
- تقارير الأداء
- كلمة سر للوصول لوضع المطور

للعودة إلى وضع المحادثة العادي، اكتب "العودة".
"""
        return features

    def _generate_settings_menu(self) -> str:
        """
        توليد قائمة بإعدادات الشات بوت القابلة للتعديل
        
        :return: نص يحتوي على قائمة الإعدادات
        """
        settings_menu = f"""
⚙️ قائمة إعدادات شات بوت مجمع عمال مصر ⚙️

✅ الإعدادات الحالية:
- استمرارية المحادثة: {'مفعلة' if self.continue_conversation else 'غير مفعلة'}
- شخصنة الردود: {'مفعلة' if self.personalize_response else 'غير مفعلة'}
- حفظ المحادثات: {'مفعل' if BOT_SETTINGS.get("SAVE_CONVERSATIONS", True) else 'غير مفعل'}
- حد التشابه للبحث: {self.similarity_threshold}

✨ أوامر تغيير الإعدادات:
- تفعيل استمرارية المحادثة: 'تفعيل الاستمرارية'
- تعطيل استمرارية المحادثة: 'تعطيل الاستمرارية'
- تفعيل شخصنة الردود: 'تفعيل الشخصنة'
- تعطيل شخصنة الردود: 'تعطيل الشخصنة'
- تفعيل حفظ المحادثات: 'تفعيل الحفظ'
- تعطيل حفظ المحادثات: 'تعطيل الحفظ'
- تعيين حد التشابه: 'تعيين التشابه 0.4' (قيمة بين 0.1 و 0.9)

⚠️ ملاحظات:
- تنطبق التغييرات فوراً ولكنها تعود للقيم الافتراضية عند إعادة تشغيل الخادم
- لتعديل الإعدادات بشكل دائم، يجب تعديل ملف config.py
- للعودة إلى وضع المحادثة العادي، اكتب "العودة"

مثال: لتعيين حد التشابه إلى 0.5، اكتب:
تعيين التشابه 0.5
"""
        return settings_menu

    def process_developer_settings(self, user_id: str, message: str) -> Optional[str]:
        """
        معالجة طلبات تغيير الإعدادات من المطور
        
        :param user_id: معرف المستخدم
        :param message: رسالة المستخدم
        :return: رد يوضح نتيجة تغيير الإعدادات أو None إذا لم تكن الرسالة تتعلق بالإعدادات
        """
        message = message.strip()
        
        # استمرارية المحادثة
        if message in ["تفعيل الاستمرارية", "تفعيل استمرارية المحادثة"]:
            self.continue_conversation = True
            return "✅ تم تفعيل استمرارية المحادثة بنجاح. سيتم إضافة عبارات للاستمرار في نهاية الردود."
        
        if message in ["تعطيل الاستمرارية", "تعطيل استمرارية المحادثة"]:
            self.continue_conversation = False
            return "✅ تم تعطيل استمرارية المحادثة بنجاح. لن يتم إضافة عبارات للاستمرار في نهاية الردود."
        
        # شخصنة الردود
        if message in ["تفعيل الشخصنة", "تفعيل شخصنة الردود"]:
            self.personalize_response = True
            return "✅ تم تفعيل شخصنة الردود بنجاح. سيتم تخصيص الردود حسب اسم المستخدم."
        
        if message in ["تعطيل الشخصنة", "تعطيل شخصنة الردود"]:
            self.personalize_response = False
            return "✅ تم تعطيل شخصنة الردود بنجاح. لن يتم تخصيص الردود حسب اسم المستخدم."
        
        # حفظ المحادثات
        if message in ["تفعيل الحفظ", "تفعيل حفظ المحادثات"]:
            BOT_SETTINGS["SAVE_CONVERSATIONS"] = True
            return "✅ تم تفعيل حفظ المحادثات بنجاح. سيتم حفظ المحادثات في ملفات JSON."
        
        if message in ["تعطيل الحفظ", "تعطيل حفظ المحادثات"]:
            BOT_SETTINGS["SAVE_CONVERSATIONS"] = False
            return "✅ تم تعطيل حفظ المحادثات بنجاح. لن يتم حفظ المحادثات في ملفات JSON."
        
        # تعيين حد التشابه
        if message.startswith("تعيين التشابه"):
            try:
                similarity_value = float(message.split()[-1])
                if 0.1 <= similarity_value <= 0.9:
                    self.similarity_threshold = similarity_value
                    return f"✅ تم تعيين حد التشابه بنجاح إلى {similarity_value}."
                else:
                    return "❌ يجب أن تكون قيمة حد التشابه بين 0.1 و 0.9."
            except (ValueError, IndexError):
                return "❌ تنسيق غير صحيح. المثال الصحيح: 'تعيين التشابه 0.5'."
        
        # لم يتم العثور على طلب تغيير إعدادات
        return None
